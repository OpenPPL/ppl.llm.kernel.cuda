    ...
    void* output) // (S, .., D)
{
    ...
    bool use_infinity_gqca = decoding_batches > 0 && (
        kv_repeats == 4 || kv_repeats == 6 || kv_repeats == 8 || kv_repeats == 16);
    ...
    const int32_t kv_length_base = use_infinity_gqca ? 2048 : 1024;
    const float kv_length_scale = std::max(1.0f, logf((float)max_kvlen / kv_length_base));
    ...
    if (decoding_attention_total_blocks < multi_processor_count * 0.9f &&
        ...
        if (use_infinity_gqca) {
            ...
            if (head_dim == 128 && (attn_mask || kv_repeats > 4)) {
                num_blocks_per_sm = 2;
            }
        } else {
    ...
    config.use_infinity_gqca = use_infinity_gqca;
