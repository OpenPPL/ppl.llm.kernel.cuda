...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    // Tensor for global bias
    const index_t h_offset_bias = params.bias_batch_stride * bidb + params.bias_head_stride * bidh;
    Tensor gBias = make_tensor(make_gmem_ptr(reinterpret_cast<Element*>(params.bias_ptr) + h_offset_bias),
    ...
    for (int mi = 0; mi < size<0>(acc_o_rowcol); ++mi) {
        ...
        float scale = inv_sum;//!Is_dropout ? inv_sum : inv_sum * params.rp_dropout;
        #pragma unroll
