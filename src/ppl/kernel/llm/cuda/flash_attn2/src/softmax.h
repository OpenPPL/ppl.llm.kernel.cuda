...
/** Applying bias to scores.
 *  The full Bias is of shape (batch, num_heads, seqlen_q, seqlen_k)
 *  the bias tensor here is the sub-tensor of corresponding batch & head.
 *
 *  NOTE: For dynamic batch with variable seq length, use max_seqlen_{q,k} for shape;
 *  NOTE 2: Bias may be broadcasted along batch/heads, but it should be treated during
 *          calculating offset of current sub-tensor with proper stride.
 *
 *  TODO: Bias is supposed to be with dtype fp16, maybe access with wider dtype?
*/
template <typename ScoreEngine, typename ScoreLayout, typename BiasEngine, typename BiasLayout>
inline __device__ void apply_bias(
    Tensor<ScoreEngine, ScoreLayout> &score,
    Tensor<BiasEngine, BiasLayout> &bias,
    ...
    const float scale_softmax_rcp)
{
    ...
    #pragma unroll
    for (int mi = 0; mi < size<0, 1>(score); ++mi) {
        const int row_idx_base = row_idx_offset + mi * warp_row_stride;
        #pragma unroll
        for (int i = 0; i < size<0, 0>(score); ++i) {
            const int row_idx = row_idx_base + i * 8;
            #pragma unroll
            for (int nj = 0; nj < size<1, 1>(score); ++nj) {
                const int col_idx_base = col_idx_offset + nj * 8;
                #pragma unroll
                for (int j = 0; j < size<1, 0>(score); ++j) {
                    const int col_idx = col_idx_base + j;
