...
// NOTE: if Has_bias=false, setting BiasTensorType to any type is OK.
//       if Has_bias=true , BiasTensorType should be exactly the bias Tensor type to be used.
template <bool Is_causal, bool Is_local, bool Has_alibi, bool Has_bias=false, typename BiasTensorType=int>
struct Mask {
    const int max_seqlen_k, max_seqlen_q;
    const int window_size_left, window_size_right;
    const float alibi_slope;
    BiasTensorType bias_tensor;
    float scale_bias;      // TODO: scaling bias before attn?
    ...
    //
    __forceinline__ __device__ void setBias(BiasTensorType t_bias, const float scale=1.0f)
    {
    ...
    __forceinline__ __device__ void apply_mask(Tensor<Engine, Layout> &tensor_,
        ...
        static constexpr bool Need_masking = Has_alibi || Causal_mask || Is_local || !Is_even_MN || Has_bias;
        ...
        if constexpr (Need_masking) {
            ...
            // NOTE: bias always needs row & col idx
            static constexpr bool Col_idx_only = !(Has_alibi && !Is_causal) && !Is_local && !Causal_mask && !Has_bias;
            const int lane_id = threadIdx.x % 32;
            const int col_idx_offset = col_idx_offset_ + (lane_id % 4) * 2;
                    ...
                    for (int i = 0; i < size<0, 0>(tensor); ++i) {
                        ...
                        for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                            ...
                            for (int j = 0; j < size<1, 0>(tensor); ++j) {
                                ...
                                // TODO: use coalesced fp16 access? currently bias is with dtype fp16.
                                if constexpr (Has_bias){
                                    tensor(make_coord(i, mi), make_coord(j, nj)) += bias_tensor(row_idx, col_idx) * scale_bias;
                                }
