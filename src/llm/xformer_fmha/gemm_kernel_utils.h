...
#define DISPATCH_TYPES(tensor, func)                                           \
  {                                                                            \
    if (query_shape->GetDataType() == ppl::common::DATATYPE_FLOAT32) {                        \
      using scalar_t = float;                                                  \
      func();                                                                  \
    } else if (query_shape->GetDataType() == ppl::common::DATATYPE_FLOAT16) {                  \
      using scalar_t = cutlass::half_t;                                        \
      func();                                                                  \
    } else if (query_shape->GetDataType() == ppl::common::DATATYPE_BFLOAT16) {              \
      using scalar_t = cutlass::bfloat16_t;                                    \
      func();                                                                  \
    } else {                                                                   \
      LOG(ERROR) << "Only fp32, half & bf16 supported at the moment";          \
...
#define DISPATCH_ARCHTAG(CC, func)                                        \
  {                                                                       \
    if (CC >= 80) {                                                       \
      ...
      return ppl::common::RC_UNSPUUORTED;                                 \
    }                                                                     \
  }
